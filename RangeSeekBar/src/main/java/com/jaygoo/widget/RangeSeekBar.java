...
public class RangeSeekBar extends View {
    ...
    //(x,y)是否在view的区域内
    private boolean isTouchPointInView(SeekBar bar, float x, float y) {
        if (bar == null) {
            return false;
        }

        int distance=(int)(bar.currPercent*progressWidth);

        int left = bar.left+distance;
        int top = bar.top;
        int right = bar.right+distance;
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (seekBarMode == SEEKBAR_MODE_RANGE) {
                    ...
                    if (rightSB.currPercent >= 1 && leftSB.collide(touchDownX, touchDownY)) {
                        currTouchSB = leftSB;
                        scaleCurrentSeekBarThumb();
                    } else if (rightSB.collide(touchDownX, touchDownY)) {
                        currTouchSB = rightSB;
                        scaleCurrentSeekBarThumb();
                    } else {
